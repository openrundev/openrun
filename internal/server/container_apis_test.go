...
func TestLogReadErrorAsLine(t *testing.T) {
	// The log viewer gets read failures as a final line, not a cut stream
	var got []any
	yield := logReadErrorAsLine(func(v any, err error) bool {
		if err != nil {
	...
	yield("line", nil)
	yield(nil, errors.New("error reading output: pipe broke"))
	if len(got) != 2 || got[0] != "line" || got[1] != "error reading output: pipe broke" {
