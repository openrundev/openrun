...
- **Use `synchronous=NORMAL`**: in WAL mode, [`PRAGMA synchronous=NORMAL`](https://www.sqlite.org/pragma.html#pragma_synchronous) is significantly faster than the default `FULL` and preserves database consistency, but recently committed transactions can be lost after power failure; it is the setting [recommended by Litestream](https://litestream.io/tips/).
...
- Deleting an app removes its managed volumes (PVCs on Kubernetes). Replicas in object storage are retained. Deleting a detached binding does not remove runtime volumes, but recreating the binding gives it a new identity and replica location.
...
Restore is automatic: if the app starts with an empty or recreated volume, the database is restored from the replica before the app starts. The server's own metadata can be replicated the same way. Recovery also requires the server config, credentials and encryption key; see [Recovery]({{< ref "docs/applications/litestream/#operational-notes" >}}). Monitor replication with `openrun replication status`.
...
- With replication enabled, OpenRun adds a restore init container and a native Litestream sidecar to the app pod automatically. Kubernetes 1.29 or newer is required for the sidecar lifecycle, which lets the sidecar perform a final sync after the app stops during graceful shutdown. Abrupt node loss or storage outages can leave recent changes unreplicated.
...
Yes. The volume is reused across app updates, redeploys and idle restarts. Deleting the app removes its managed volume; replicas in object storage are retained.
