...
For the `system` and `builtin` auth types, browser access to an app shows an HTML login page (stating whether the app uses the system admin account or OpenRun builtin accounts) and redirects back to the original app URL after sign in. The authenticated session is stored in a cookie on the app's domain; its lifetime is `security.session_max_age`, the same as SSO sessions. The lifetime is an idle timeout: a session that is in use is renewed (once it is past half of its lifetime), so it only expires after `session_max_age` without activity. `security.session_absolute_max_age` (default one week) caps the total lifetime from login regardless of activity; set it to 0 to disable the cap. Both settings apply to all session-based auth types and are read at startup. The login page itself stays valid for one hour: a form submitted later than that (or submitted twice) shows a "sign-in request has expired" message with a link back to the app, which starts a fresh sign in. Sessions are re-validated against the live config on every request: changing or removing a user's password, deleting a builtin user or changing the admin username signs the affected sessions out immediately. Signing out is done through the `/_openrun/logout` page on the app domain (see below).
