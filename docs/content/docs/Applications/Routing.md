...
- `/_openrun_app/verify_file/{file_name}` verifies that an app-relative source file exists and returns JSON with `file_path`, `file_exists`, and `file_size`. The file name is validated before lookup, so absolute paths and parent directory references are rejected.
